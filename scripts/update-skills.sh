...
# Define repositories as name:url pairs
REPOS=(
    ...
    "chrisroyse-subagents:https://github.com/ChrisRoyse/610ClaudeSubagents.git"
...
new_agents=0
for source in feiskyer lst97 chusri rahulvrane nicholasspisak chrisroyse; do
    for repo in "$REPOS_DIR"/${source}*; do
        if [ -d "$repo" ]; then
...
if [ -d "$REPOS_DIR/wshobson-agents" ]; then
    ...
    cp -r "$REPOS_DIR/wshobson-agents/plugins" "$PLUGINS_DIR/wshobson-agents/" 2>/dev/null || true
    echo -e "  ${GREEN}✓${NC} wshobson-agents marketplace (67 plugins)"
fi

# Update rsmdt/the-startup marketplace
if [ -d "$REPOS_DIR/rsmdt-the-startup" ]; then
    mkdir -p "$PLUGINS_DIR/rsmdt-the-startup"
    cp -r "$REPOS_DIR/rsmdt-the-startup"/* "$PLUGINS_DIR/rsmdt-the-startup/" 2>/dev/null || true
    echo -e "  ${GREEN}✓${NC} rsmdt-the-startup marketplace"
fi
